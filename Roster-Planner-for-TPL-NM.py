...
wb = openpyxl.load_workbook('ROSTER_INPUT.xlsx')
sheet = wb.active
...
for i in range(totaldays):  # writing dates and weekdays in excel 1st and 2nd row
    ...
    sheet1.cell(row=1, column=i+2).font = sheet1.cell(row=2,
                                                      column=i+2).font = myfont
    sheet1.column_dimensions[get_column_letter(i+2)].width = 5
    if weekday > 7:
        weekday = 1
    if weekday == 1:
        sheet1.cell(row=2, column=i+2).value = 'SUN'
    elif weekday == 2:
        sheet1.cell(row=2, column=i+2).value = 'MON'
    elif weekday == 3:
        sheet1.cell(row=2, column=i+2).value = 'TUE'
    elif weekday == 4:
        sheet1.cell(row=2, column=i+2).value = 'WED'
    elif weekday == 5:
        sheet1.cell(row=2, column=i+2).value = 'THU'
    elif weekday == 6:
        sheet1.cell(row=2, column=i+2).value = 'FRI'
    elif weekday == 7:
        sheet1.cell(row=2, column=i+2).value = 'SAT'
    weekday = weekday+1
...
wb1.save('NM_ROSTER_PLANNED_BY_SATISH.xlsx')
